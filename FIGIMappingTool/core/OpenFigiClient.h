...
public:
  ...
  [[nodiscard]] Result<std::vector<MappingResult>> map(std::string_view id_value,
  ...
  [[nodiscard]] Result<std::vector<Instrument>> lookup_ticker(std::string_view ticker,
                                                              std::optional<std::string_view> exch_code = std::nullopt);
...
private:
  ...
  [[nodiscard]] Result<HttpResponse> post(std::string_view endpoint, const nlohmann::json& body, RateLimiter& limiter);

  [[nodiscard]] Result<HttpResponse> get(std::string_view endpoint, RateLimiter& limiter);

  [[nodiscard]] Result<HttpResponse> request_with_retry(std::function<Result<HttpResponse>()> do_request,
                                                        RateLimiter& limiter);
