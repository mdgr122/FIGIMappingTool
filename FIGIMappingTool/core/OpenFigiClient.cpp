...
Instrument Instrument::from_json(const nlohmann::json& j) {
  ...
  return {
      ...
      .name = get("name"),
      .ticker = get("ticker"),
      .exchCode = get("exchCode"),
      .securityType = get("securityType"),
      .marketSector = get("marketSector"),
      .securityType2 = get("securityType2"),
      .securityDescription = get("securityDescription"),
      .compositeFIGI = get("compositeFIGI"),
      .shareClassFIGI = get("shareClassFIGI"),
  };
}
...
RateLimiter::RateLimiter(int max_requests, std::chrono::seconds window) : m_max(max_requests), m_window(window) {
...
size_t OpenFigiClient::batch_size() const {
  return current_api_key().empty() ? 10 : 100;
}
...
Result<OpenFigiClient::HttpResponse> OpenFigiClient::post(std::string_view endpoint, const nlohmann::json& body,
                                                          RateLimiter& limiter) {
  auto api_key = current_api_key();
  return request_with_retry(
      [&, api_key]() -> Result<HttpResponse> {
        ...
        if (r.error) {
          return std::unexpected(Error{ErrorCode::NetworkError, std::format("Network error: {}", r.error.message), {}});
        }
    ...
    if (!arr.is_array() || arr.size() != (end - offset)) {
      return std::unexpected(
          Error{ErrorCode::ParseError,
                std::format("Expected {} results, got {}", end - offset, arr.is_array() ? arr.size() : 0), arr.dump()});
    }
...
Result<std::vector<MappingResult>> OpenFigiClient::map(std::string_view id_value,
...
Result<std::vector<Instrument>> OpenFigiClient::lookup_figi(std::string_view figi) {
  return extract_first_data(map(std::vector{MappingJob{.idType = IdType::ID_BB_GLOBAL, .idValue = std::string(figi)}}));
}

Result<std::vector<Instrument>> OpenFigiClient::lookup_ticker(std::string_view ticker,
                                                              std::optional<std::string_view> exch_code) {
