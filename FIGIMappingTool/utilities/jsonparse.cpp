...
std::string JsonParse::build_csv_row(const std::vector<std::string>& header_keys,
                                     const std::map<std::string, std::string>& main_values,
                                     const std::map<std::string, std::string>& data_values) const
{
    std::string row_line;
    int key_count = 0;
    for (const auto& key : header_keys)
    {
        ...
        // Added the key_count check because if there are duplicate keys in the request and response, its set to take value of request first if this check is not in place.
        {
